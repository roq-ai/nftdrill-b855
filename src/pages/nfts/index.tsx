...
import { getNfts, deleteNftById } from 'apiSdk/nfts';
import { NftInterface } from 'interfaces/nft';

type ColumnType = ColumnDef<NftInterface, unknown>;
...
export function NftListPage(props: NftListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<NftInterface>>(
    () => `/nfts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: NftInterface) => {
    if (hasAccess('nft', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/nfts/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/nfts/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('nft', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/nfts/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('nft', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="nft" />
        </Flex>

        {hasAccess('nft', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/nfts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
